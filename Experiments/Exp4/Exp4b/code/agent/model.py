...
class myModel(object):
    ...
    def __init__(self,
        ...
        if not(is_trained):
            ...
            with tf.name_scope('input'):
                self.x = self.imported_graph.get_operation_by_name("input/x_in").outputs[0]   #tf.placeholder(tf.float32,[None,self.dim_inputs[1]*self.dim_inputs[2]*self.dim_inputs[3]],name='x_in')
    ...
    def nnet_builder(self):
        ...
        with tf.variable_scope('convolutions'):
            ...
            # import the last convolutional layer from the cvae
            self.imported_encoder = tf.stop_gradient(self.imported_graph.get_operation_by_name("nnet/encoder/l3_conv/Relu").outputs[0])
            print(self.imported_encoder.get_shape().as_list())
        # fully-connected block
        with tf.variable_scope('fullyconnected'):  
            # 1. flatten       
            self.l_flat = layer_flatten(self.imported_encoder,name='l_flat')
    ...
    def readLayers(self,x):
        ...
        fc1 = self.session.run(self.l_fc,feed_dict={self.x: x,self.keep_prob:1.0})
    ...
    def images(self,x):
        if x.ndim==1:
            x = np.expand_dims(x,axis=0)
        imgOrig = self.session.run(self.summaryImgOrig,

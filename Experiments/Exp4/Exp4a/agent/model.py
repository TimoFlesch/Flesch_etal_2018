"""
implements a convolutional beta VAE
Timo Flesch, 2017
"""
...
class myModel(object):
    ...
    def __init__(self,
        ...
        if not(is_trained):
            with tf.name_scope('input'):
                ...
                self.y_true = tf.placeholder(tf.float32,[None,self.dim_inputs[1]*self.dim_inputs[2]*self.dim_inputs[3]],name='y_true')
            ...
            with tf.name_scope('reshaper'):
                self.visIMG_orig  = tf.reshape(self.x_img,[1,96,96,3],name='reshape_input')
                self.visIMG_recon =tf.reshape(self.y_hat,[1,96,96,3],name='reshape_output')
    ...
    def nnet_builder(self):
        ...
        with tf.variable_scope('decoder'):   
            ...
            self.l6_transconv, self.params['l6_transconv_weights'], self.params['l6_transconv_biases'], self.params['l6_transconv_shape'] = layer_transpose_conv2d(self.l5_transconv,
                16,(5,5),shape=self.params['l1_conv_shape'],name='l6_transconv',nonlinearity=tf.nn.tanh,stride=(2,2),padding='SAME')
    ...
    def optimizer_builder(self,optimizer):
        ...
        with tf.name_scope('loss-function'):
            ...
            with tf.name_scope('reconstr_loss'):
                # self.reconstr_loss = tf.sqrt(tf.reduce_sum(tf.pow(self.y_true-self.y_hat,2),1))     
            ...
            with tf.name_scope('latent_loss'):
                self.latent_loss  =  self.beta * -0.5 * tf.reduce_sum(1 + self.layer_z_logvar - tf.square(self.layer_z_mu) - tf.exp(self.layer_z_logvar),1)
    ...
    def init_graph_vars(self,sess,log_dir='.'):
        ...
        # define summaries
        self.summaryLossRecon = tf.summary.scalar("loss_recon",tf.reduce_mean(self.reconstr_loss))
    ...
    def readLayers(self,x):
        ...
        if x.ndim==1:
            x = np.expand_dims(x,axis=0)            
        enc = self.session.run(self.enc_flatten,feed_dict={self.x: x})        
        latent = self.session.run(self.layer_z,feed_dict={self.x: x})
        return enc,latent


    def train(self,x,y_true):
        """ training step """ 
        _,loss,summary_train,summary_latent,summary_recon,summary_conv = self.session.run([self.train_step,self.total_loss,self.summaryTraining,self.summaryLossLatent,self.summaryLossRecon,self.summaryImgConv],
        ...
        self.writer.add_summary(summary_conv,self.ITER)

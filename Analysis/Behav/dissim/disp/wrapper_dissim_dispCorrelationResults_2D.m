function wrapper_dissim_dispCorrelationResults_2D()
	%% WRAPPER_DISSIM_DISPCORRELATIONRESULTS_2D
	%
	...
	for cIDX = 1:length(curricula)
		for bIDX = 1:length(boundary_groups)
			...
			% load modelcorrs - Pre
			load(['tauas_2D_modelRDM_correlations_' boundary_groups{bIDX}  '_group' curricula{cIDX} '_' 'pre' '_premainpost_allindia.mat']);
			taus_pre = tauas_2D;

			% load modelcorrs - Post
			load(['tauas_2D_modelRDM_correlations_' boundary_groups{bIDX}  '_group' curricula{cIDX} '_' 'post' '_premainpost_allindia.mat']);
			taus_post = tauas_2D;
			...
			% call funct
			dissim_dispCorrelationResults_1D2D(taus_pre,taus_post,noiseceil_pre,noiseceil_post,titleStr,2);

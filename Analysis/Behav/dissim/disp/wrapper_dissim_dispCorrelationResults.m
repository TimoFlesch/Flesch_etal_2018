function wrapper_dissim_dispCorrelationResults()
	%% WRAPPER_DISSIM_DISPCORRELATIONRESULTS
	%
	...
	for cIDX = 1:length(curricula)
		for bIDX = 1:length(boundary_groups)
			...
			load(['tauas_features_modelRDM_correlations_' boundary_groups{bIDX}  '_group' curricula{cIDX} '_' 'post' '_premainpost_allindia.mat']);
			taus_post = tauas_features;
			...
			% call funct
			dissim_dispCorrelationResults(taus_pre,taus_post,noiseceil_pre,noiseceil_post,titleStr);

function disp_choicemodel_offsets_decisionBound(results,boundaryCode,subGroup)
	 %% DISP_CHOICEMODEL_OFFSETS_DECISIONBOUND(ANGLES,BOUNDARYCODE)
	 %
	 % plots errorbarplots of mean offset of decision boundary to category boundary for all groups
	 %
	 % Timo Flesch, 2017
	...
	% Offset - Training
	% helper_barplots_offset(results.train,'Training',boundaryCode,subGroup);

	% Offset - Test
	helper_barplots_offset(results.test,'Test',boundaryCode,subGroup);

end

function helper_barplots_offset(results,sessionCode,boundaryCode,subGroup)
	%
	...
	for ii = 1:length(angleFields)
		results.(angleFields{ii}) = results.(angleFields{ii}).offset;
	end
	...
	title({['\bf Mean Offset of Decision Boundary - ' sessionCode]; ['\rm' boundaryCode ' boundary']});
	ylim([-0.5,0.5]);
	set(gca,'Box','off');
